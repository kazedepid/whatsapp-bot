    ...
    try {
        ...
        function watchDirectory(directory) {
            fs.readdir(directory, (err, files) => {
                if (err) return console.log(func.colors(err, 'bgRed'))
                files.forEach(file => {
            ...
            try {
                ...
                if (eventType === 'rename') {
                    if (fs.existsSync(filePath)) {
                        ...
                        console.log(func.colors(`Plugin '${file}' added.`, 'green'))
                    } else {
                        delPlugin(file)
                        console.log(func.colors(`Plugin '${file}' removed.`, 'red'))
                    }
                } else if (eventType === 'change') {
                    await addPlugin(filePath, file)
                    console.log(func.colors(`Plugin '${file}' changed.`, 'cyan'))
                }
            } catch (e) {
                console.log(func.colors(`Plugin '${file}' error.`, 'bgRed'))
            }
        }
            ...
            for (const file of files) {
                ...
                if (fs.statSync(filePath).isDirectory()) {
                    ...
                    try {
                        ...
                        console.log(func.colors(`${e}: Plugin '${file}' error.`, 'bgRed'))
                    }
                }
            }
        }
