...
async function connectSock() {
    ...
    console.log(func.colors(`Using WA v${version.join('.')}, isLatest: ${isLatest}`, 'cyan'))
    ...
    if (usePairingCode && !conn.authState.creds.registered) {
        setTimeout(async () => {
            rl.question(
                `${func.colors('\nMasukkan nomor telepon untuk wa bot dengan seperti ini 6282xxxxxxxx.', 'cyan')}\n${func.colors('Nomor', 'rainbow')}: `,
                async function (phoneNumber) {
                    ...
                    console.log(`\n${func.colors('Code', 'green')} : ${code.match(/.{1,4}/g)?.join('-')}\n`)
                    rl.close()
                }
            )
        }, 3000)
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'open') {
            console.log(func.colors(`Terhubung`, 'green'))
        ...
        if (connection === 'close') {
            console.log(func.colors(`Disconnecting`, 'red'))
            ...
            if (shouldReconnect) {
                console.log(func.colors(`Disconnecting`, 'cyan'))
                connectSock()
            }
        }
    })
